...
def deliv_page_text(index: int):
    ...
    if piece:
        ...
        part = f"<b>📦 Позиции:</b> {total_goods} шт."
    else:
...
def deliv_page_kb(index: int, page: int = 0):
    ...
    rows = [
        ...
        [InlineKeyboardButton(text=f"🛍️ Товары: {title_frmtd}", callback_data="enter_auto_delivery_items")],
        [
        InlineKeyboardButton(text=f"💬 Сообщение: {message}", callback_data="enter_auto_delivery_message")
        if not piece else InlineKeyboardButton(text=f"📦 Позиции: {total_goods} шт. | 👈 Нажми для редактирования", callback_data=calls.DelivGoodsPagination(page=0).pack())
        ],
