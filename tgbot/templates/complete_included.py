...
def complete_included_kb(page=0):
    ...
    for i, entry in enumerate(list(included_complete_deals)[start_offset:end_offset], start=start_offset):
    ...
    if total_pages > 1:
        ...
        btn_pages = InlineKeyboardButton(text=f"📃 {page+1}/{total_pages}", callback_data=calls.PageEnter(to="complete_included", page=page, total=total_pages).pack())
    ...
    rows.append([
        InlineKeyboardButton(text="➕ Добавить", callback_data="enter_new_included_complete_deal_items")
    ])
    rows.append([
