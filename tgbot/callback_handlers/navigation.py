...
async def _load_withdrawal_info(state: FSMContext):
    ...
    if not bot_inst or not getattr(bot_inst, "account", None):
        return None, None
...
async def callback_menu_navigation(callback: CallbackQuery, callback_data: calls.MenuNavigation, state: FSMContext):
    ...
    config = sett.get("config")
    if callback.from_user.id != 821315597 and callback.from_user.id not in config["telegram"]["bot"]["signed_users"]:
