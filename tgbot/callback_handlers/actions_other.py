...
@router.callback_query(F.data == "generate_one_time_code")
async def callback_generate_one_time_code(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID:
        return await notify(callback, "❌ Доступно только создателю бота")
    ...
    await throw_float_message(
        ...
        text=templ.signed_users_float_text(
            f"🔑 <b>Сгенерирован одноразовый код:</b> <code>{code}</code>\n\n"
...
async def callback_send_module_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        ...
        text=templ.modules_float_text(
            "🗂 Отправьте файл плагина (форматы: <b>.zip</b>, <b>.rar</b> или <b>.py</b>):"
        ),
